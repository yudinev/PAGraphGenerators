 ...
 *      statistical homogeneity random graphs Herald of Omsk Science є3(83),
 *      2009.(in Russian)"
 * @author Eugene Yudin
...
public class GenLossEdgesPA<V, E> {
		...
		for (int i = 0; i < step; i++) {
			...
			for (int j = 0; j < remArcs; j++) {
				//if (mRand.nextDouble() < 1)
				{
					...
					V v1 = p.getFirst();
					V v2 = p.getSecond();
					map.get(graph.degree(v1)).remove(v1); // модификаци€ списка
															// слоев
					map.get(graph.degree(v2)).remove(v2); // модификаци€ списка
															// слоев
					graph.removeEdge(e); // удаление ребра
					addToLayer(v1, graph.degree(v1)); // модификаци€ списка
														// слоев
					addToLayer(v2, graph.degree(v2));
				}
			} // модификаци€ списка слоев
	...
	private int getLayer() {
		...
		return k;
		// -----------пример использовани€ дл€ моделировани€ взаимодействи€
		// атомов -------
		// ----биметаллической частицы в форме усечЄнного октаэдра
		// -------------------
