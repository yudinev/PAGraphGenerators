 ...
 * 
 */
public class GenCalibCoefClastPA<V, E> {
	...
	protected double numEdgesToAttach2[];
	 ...
	 * 
	 */
	public GenCalibCoefClastPA(Factory<V> vertexFactory, Factory<E> edgeFactory, double[] probEdgesToAttach,
			double[] r_nonDiad, PrefferentialAttachment attachRule) {
		double s = 0.;
	 ...
	 *            "preferential attachment"
	 */
	/*public GenNonIntBA4(Factory<V> vertexFactory, Factory<E> edgeFactory, double[] numEdgesToAttach, boolean parallel,
			int seed, PrefferentialAttachment attachRule) {
		this(vertexFactory, edgeFactory, numEdgesToAttach, attachRule);
...
int addEd_a = addEd_b;
				...
				// выбираю вершины для присоединения для второй вершины
				// приращения второй вершины
				if (addEd_a > 0)
				...
				double r=mRand.nextDouble();
				int addEd =0;
				for (int j = 0; j < numEdgesToAttach2.length; j++) {
						s=s+numEdgesToAttach2[j];
						if(s>r)
						{
							addEd =	j;
							break;
